...
// ---------------------------------------------------------------------
// SupplierRegistry — ChainIntegrate
//
// SCHELETRO DI DISCUSSIONE, non pronto per il deploy. Punti volutamente
// lasciati aperti sono segnalati con "// TODO(decisione): ...".
//
// Architettura concordata:
// - 1 LSP8 (soulbound) per utente = il suo "Registro"
// - tokenId = indirizzo dell'utente che ha mintato (formato ADDRESS),
//   cosi' non serve un contatore ne' storage per il mapping owner->token
...
contract SupplierRegistry is LSP8IdentifiableDigitalAsset {
    // -----------------------------------------------------------------
    // Errori
    // -----------------------------------------------------------------
    error NoRecognizedMembership(address account);
    error RegistryAlreadyMinted(address account);
    ...
    function setTierLimits(
        ...
        emit TierLimitsSet(membershipContract, tier, maxSuppliers, maxParams, canDiscloseSelectively);
    }
    ...
    function getEffectiveLimits(address account)
        public
        view
        returns (uint256 maxSuppliers, uint256 maxParams, bool canDiscloseSelectively)
    {
        uint256 len = _membershipContracts.length;
        for (uint256 i = 0; i < len; i++) {
    ...
    function _hasAnyRecognizedTier(address account) internal view returns (bool) {
        (uint256 maxSuppliers, , ) = getEffectiveLimits(account);
        return maxSuppliers > 0;
    }

    // -----------------------------------------------------------------
    // Registro — 1 LSP8 soulbound per utente, tokenId = indirizzo
    // -----------------------------------------------------------------

    event RegistryMinted(bytes32 indexed tokenId, address indexed owner);
    ...
    /// @notice Chiunque puo' mintare il proprio Registro, a patto di
    ///         avere almeno un tier riconosciuto su un contratto
    ///         Membership accettato (oggi: almeno Bronze).
    function mintRegistry() external {
        bytes32 tokenId = bytes32(uint256(uint160(msg.sender)));
        if (_exists(tokenId)) revert RegistryAlreadyMinted(msg.sender);
        if (!_hasAnyRecognizedTier(msg.sender)) revert NoRecognizedMembership(msg.sender);

        _mint(msg.sender, tokenId, true, "");
        emit RegistryMinted(tokenId, msg.sender);
    }
