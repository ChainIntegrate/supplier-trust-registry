...
// ---------------------------------------------------------------------
// SupplierRegistry — ChainIntegrate — V3
//
// V3 rispetto alla V2 (0x527F333d351E5A9b3061f59ED338631aabdB2C93, gia'
// live su testnet — vedi SupplierRegistry-v2.sol): aggiunge un'immagine
// personalizzata per registro (feature Silver+), tramite una funzione
// custom (non lo standard LSP4 setDataForTokenId, che sarebbe onlyOwner
// del contratto, non del singolo registro — vedi sezione dedicata piu'
// sotto per il dettaglio). Tutto il resto invariato rispetto alla V2.
//
// V2 rispetto al contratto originale (vedi SupplierRegistry-v1.sol):
...
contract SupplierRegistryV3 is LSP8IdentifiableDigitalAsset {
    // -----------------------------------------------------------------
    // Errori
    // -----------------------------------------------------------------
    ...
    struct TierLimits {
        ...
        bool canCustomizeImage; // feature Silver+: immagine personalizzata del registro
    ...
    function setTierLimits(
        ...
        tierLimits[membershipContract][tier] = TierLimits({
            ...
            canCustomizeImage: canCustomizeImage,
            configured: true
        });
        emit TierLimitsSet(membershipContract, tier, maxSuppliers, maxParams, canDiscloseSelectively, maxRegistries, canCustomizeImage);
    }
    ...
    // -----------------------------------------------------------------
    // Immagine del registro (feature Silver+) — NON usa il meccanismo
    // standard LSP4/setDataForTokenId, che e' onlyOwner del CONTRATTO
    // (la UP ChainIntegrate), non del singolo registro: con quello, solo
    // ChainIntegrate potrebbe impostare l'immagine per conto degli utenti,
    // mai loro stessi in self-service. Stessa funzione custom + stesso
    // onlyRegistryOwner gia' usato per fornitori/valutazioni/schema.
    //
    // Valore mutabile (non append-only come il resto): ha senso qui,
    // un'immagine si sostituisce, non si accumula in versioni storiche.
    // Emette comunque un evento, coerente con lo stile del contratto,
    // ma la lettura "qual e' l'immagine attuale" passa dal getter diretto,
    // non da una scansione di log — piu' semplice per un valore singolo
    // che deve sempre riflettere solo l'ultimo stato.
    // -----------------------------------------------------------------
    ...
    mapping(bytes32 => RegistryImage) private _registryImage;

    event RegistryImageUpdated(bytes32 indexed tokenId, string uri, bytes32 hash);

    error ImageCustomizationNotAllowed(address account);

    function setRegistryImage(
        bytes32 tokenId,
        string calldata uri,
        bytes32 hash
    ) external onlyRegistryOwner(tokenId) {
        (, , , , bool canCustomizeImage) = getEffectiveLimits(msg.sender);
        if (!canCustomizeImage) revert ImageCustomizationNotAllowed(msg.sender);

        _registryImage[tokenId] = RegistryImage({ uri: uri, hash: hash });
        emit RegistryImageUpdated(tokenId, uri, hash);
    }

    function getRegistryImage(bytes32 tokenId)
        external
        view
        returns (string memory uri, bytes32 hash)
    {
        RegistryImage memory img = _registryImage[tokenId];
        return (img.uri, img.hash);
    }
