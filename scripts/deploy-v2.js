...
const TIER_LIMITS = [
  { tier: 1, label: "Bronze", maxSuppliers: 5, maxParams: 4, canDiscloseSelectively: false, maxRegistries: 1 },
  { tier: 2, label: "Silver", maxSuppliers: 25, maxParams: 8, canDiscloseSelectively: false, maxRegistries: 2 },
  { tier: 3, label: "Gold", maxSuppliers: 100, maxParams: 1000, canDiscloseSelectively: true, maxRegistries: 5 },
];
...
async function main() {
  ...
  // -----------------------------------------------------------------
  const SupplierRegistryV2 = await hre.ethers.getContractFactory("SupplierRegistryV2");
  const registry = await SupplierRegistryV2.deploy(
